# 「小説を読む → 映画化を観る → 舞台の街の店に行く → そこで食べたものを再現する」
...
visit = Recorder.start(
  subject: { kind: "place", title: "芦屋の割烹", creator: "和食", lat: 34.7275, lng: 135.3050 },
  event: { type: "did", occurred_on: Date.new(2026, 3, 21), rating: 5, caused_by: watch.id, note: "花見の帰りに。鯛の子が出た" }
)

Recorder.start(
  subject: { kind: "dish", title: "鯛の子の煮付け", creator: "細雪の食卓" },
  event: { type: "wished", occurred_on: Date.new(2026, 3, 22), caused_by: visit.id, note: "店で食べた鯛の子を家で" }
)

# 溜まっていく wished の例
[ [ "book", "痴人の愛", "谷崎潤一郎" ], [ "film", "鍵", "市川崑" ], [ "place", "元町の洋食屋", "洋食" ],
  [ "dish", "ぼたん鍋", nil ], [ "book", "陰翳礼讃", "谷崎潤一郎" ], [ "place", "南京町の豚まん屋", "中華" ] ].each_with_index do |(kind, title, creator), i|
  Recorder.start(
