...
class EventEditTest < ActionDispatch::IntegrationTest
  ...
  def edit(event, **fields)
    patch event_path(event), params: { event: { occurred_year: "", occurred_month: "", occurred_day: "", **fields } }
  end

  test "編集ページには今の日付・評価・メモが入っている" do
    get edit_event_path(@did)
    assert_response :success
    assert_select "input[name='event[occurred_year]'][value='2019']"
    assert_select "input[name='event[occurred_month]']:not([value])"
  ...
  test "日付を直すと状態が計算し直され、詳細ページに戻る" do
    ...
    edit(@did, occurred_year: "2026", occurred_month: "9", occurred_day: "1")
  ...
  test "種別は送っても変わらない" do
    edit(@did, type: "dropped", occurred_year: "2019")
    assert_equal "did", @did.reload.type
  end

  test "暦に無い日付は 422 で編集ページに戻る" do
    edit(@did, occurred_year: "2019", occurred_month: "2", occurred_day: "30")
