...
class RecordingFlowTest < ActionDispatch::IntegrationTest
  ...
  test "本を記録すると subject と event ができ詳細に飛ぶ" do
    assert_difference [ "Subject.count", "Event.count" ], 1 do
      post books_path, params: {
        ...
        event: { type: "wished", occurred_on: "2026/1/1", note: "映画の前に" }
      }
    end
  ...
  test "タイトルが空なら 422 でフォームに戻る" do
    assert_no_difference "Subject.count" do
      post books_path, params: { subject: { title: "" }, event: { type: "wished", occurred_on: "2026/1/1" } }
    end
  ...
  test "caused_by で連鎖が辿れる" do
    post books_path, params: { subject: { title: "細雪" }, event: { type: "did", occurred_on: "2026/1/1" } }
    read = Subject.find_by!(title: "細雪").events.sole

    post films_path, params: {
      subject: { title: "細雪 (1983)", creator: "市川崑" },
      event: { type: "wished", occurred_on: "2026/1/5", caused_by: read.id }
    }
...
class ListingTest < ActionDispatch::IntegrationTest
  test "時系列は kind をまたいで一本に並び kind で絞れる" do
    post books_path, params: { subject: { title: "細雪" }, event: { type: "did", occurred_on: "2026/1/1" } }
    post places_path, params: { subject: { title: "芦屋の割烹" }, event: { type: "did", occurred_on: "2026/5/1" } }
  ...
  test "今日の候補には wished だけが出る" do
    post books_path, params: { subject: { title: "したい本" }, event: { type: "wished", occurred_on: "2026/1/1" } }
    post films_path, params: { subject: { title: "観た映画" }, event: { type: "did", occurred_on: "2026/1/1" } }
